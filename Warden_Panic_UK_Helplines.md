# Warden — Panic / Help Component: Real UK Helplines + Incident Response
### Single Claude Code prompt. Paste the whole block at once.
### All phone numbers and contacts verified against official UK sources (May 2026).

---

## WHAT THIS BUILDS

A completely redesigned Panic/Help component with:

1. **Incident First Aid** — immediate step-by-step response based on what happened
2. **Region Selector** — England / Scotland / Wales / Northern Ireland picker
   (changes which reporting authority is shown — legally different per nation)
3. **Real verified UK helplines** — hardcoded, accurate, always available offline
4. **Specialist contacts** — kids, elderly, emotional support, specific scam types
5. **"Right now" emergency flow** — still on the phone? Just clicked a link? Different first aid for each
...
```
Replace the entire contents of src/components/Panic.jsx with the following.
Also add the CSS at the bottom to src/index.css (or styles.css).

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PANIC.JSX — Full replacement
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CSS — Add to end of styles.css / index.css
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/* ════════════════════════════════════
   PANIC MODE — UK Helplines
   ════════════════════════════════════ */
...
.panic-step-group { margin-bottom: 24px; }
...
.panic-incident-btn:hover {
  border-color: var(--accent);
  transform: translateX(3px);
}

.panic-incident-btn.active {
  border-color: var(--urgency, var(--accent));
  background: color-mix(in srgb, var(--urgency, var(--accent)) 6%, var(--white));
  border-left-width: 4px;
}

.panic-incident-emoji { font-size: 20px; flex-shrink: 0; }

/* ── First aid ── */
.panic-first-aid { margin-top: 4px; }
...
[data-theme="elderly"] .panic-script-text { font-size: 18px; }
...
.panic-check-item:hover { background: var(--cream-mid); }
.panic-check-item.checked { background: rgba(76,175,130,0.08); }
...
.panic-region-btn:hover { border-color: var(--accent); }
.panic-region-btn.active {
  border-color: var(--accent);
...
@media (max-width: 768px) {
  ...
  .panic-right {
    order: -1; /* Show contacts at top on mobile */
  }
  .panic-region-grid {
...
@media (max-width: 480px) {
  .panic-incident-btn { font-size: 13px; padding: 12px 14px; }
  .panic-script-text { font-size: 16px; }
}
```
...
────────────────────           ────────────────────────────────
1. What happened?              3. Where are you?
   ○ I'm on the phone now         [🏴󠁧󠁢󠁥󠁮󠁧󠁿 England] [🏴󠁧󠁢󠁳󠁣󠁴󠁿 Scotland]
   ○ I gave bank details          [🏴󠁧󠁢󠁷󠁬󠁳󠁿 Wales  ] [🇬🇧 N.Ireland]
   ○ I transferred money
   ○ I clicked a link          🚨 REPORT TO:
   ○ I gave my password           Report Fraud — 0300 123 2040
   ○ Got a message (no reply)     reportfraud.police.uk
   ○ Not sure
                               FIRST CALL:
↓ User selects ↓                  🏦 159 — Your Bank Fraud Line
                                  24/7 · Free
"Say this now:
 [exact script in big text]"   SPECIALIST SUPPORT:
                                  🫖 Age UK — 0800 678 1602
2. Do these steps:                🌙 Silver Line — 0800 4 70 80 90
   □ 📵 Hang up now               💜 Victim Support — 0808 168 9111
   □ ⏳ Wait 5 minutes
   □ 📱 Use a different phone   [+ All UK helplines]
   □ 🔢 Call 159
...
## KEY VERIFIED NUMBERS (all confirmed May 2026)

| Service | Number | Notes |
|---|---|---|
| Bank fraud line | **159** | Works with all major UK banks |
| Report Fraud (England/Wales/NI) | **0300 123 2040** | Replaced Action Fraud Dec 2025 |
| Police Scotland | **101** | Scotland ONLY — not Report Fraud |
| Citizens Advice | **0808 223 1133** | Free, Mon-Fri |
| Age UK | **0800 678 1602** | 8am-7pm every day |
| Silver Line (55+) | **0800 4 70 80 90** | 24/7 free |
| Victim Support | **0808 168 9111** | 24/7 England & Wales |
| Childline | **0800 1111** | 24/7 under 19s |
| Samaritans | **116 123** | 24/7 emotional support |
| Report scam texts | **7726** | Forward the text |
| Report scam emails | **report@phishing.gov.uk** | Forward the email |

---

*Warden Panic Mode — Real UK helplines · Verified May 2026 · Region-aware*
